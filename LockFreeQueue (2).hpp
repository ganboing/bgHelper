...
template<class T, size_t Capacity>
void LockFreeQueue<T, Capacity>::Commit(const T* pElement)
{
	MemoryBarrier();
...
template<class T, size_t Capacity>
T* LockFreeQueue<T, Capacity>::Front()
{
	packed_ptr old_ptrs;
	old_ptrs.val = ptrval;
...
void LockFreeQueue<T, Capacity>::Dequeue()
{
	...
	} while (InterlockedCompareExchange64(&ptrval, new_ptrs.val, old_ptrs.val) != old_ptrs.val);
}

template<class T, size_t Capacity>
T* LockFreeQueue<T, Capacity>::Acquire()
{
	packed_ptr old_ptrs;
	for (;;)
	{
		...
		if (old_ptrs.size < Capacity)
		{
			...
			new_ptrs.head = old_ptrs.head;
			new_ptrs.size = old_ptrs.size + 1;
			if (InterlockedCompareExchange64(&ptrval, new_ptrs.val, old_ptrs.val) == old_ptrs.val)
			{
